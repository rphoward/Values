#!/usr/bin/env python3
"""Write product, UX, and app design briefs."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write all three design briefs.")
    parser.add_argument("session", type=Path, help="Path to session.json")
    parser.add_argument(
        "--force",
        action="store_true",
        help="Write briefs even when module gates are not all complete or bypassed",
    )
    ...
    written: list[str] = []
    for template_name, output_name in DESIGN_BRIEFS:
        output_path = args.session.parent / output_name
        content = fill_design_brief(session, template_name)
