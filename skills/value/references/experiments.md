(def-ref experiments
  ...
  (atom
    ...
    (unlocks "experiments gate: write experiment-plan.md from accepted experiment state, set its artifact status final so completion derives from the pass decision plus artifact, then unlock product-design-brief.md and ux-brief.md only when every module outcome is completed or bypassed")))
